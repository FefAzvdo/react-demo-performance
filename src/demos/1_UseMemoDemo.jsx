    ...
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <button
          ...
          className="bg-indigo-500 hover:bg-indigo-600 text-white px-5 py-2 rounded-md shadow cursor-pointer"
        >
          Click fast! Counter: {clicks}
        </button>
        <button
          onClick={() => setNumbers((prev) => [...prev, prev.length + 1])}
          className="bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-md shadow cursor-pointer"
        >
          Add number (triggers re-calculation)
        </button>
        <label className="flex items-center gap-2 mt-2 sm:mt-0 cursor-pointer">
          <input
            type="checkbox"
            checked={useMemoMode}
